// Module to handle language-related routes
module.exports = (app, db) => {
    // GET route for '/language' URL path
    app.get("/language", (req, res) => {
        ...
        db.query(sqlWorldPop, (err, worldPopResults) => {
            if (err) {
                // Log and send server error if the query fails
                console.error("Error fetching world population:", err);
                return res.status(500).send("Server Error");
            }

            // Store the total world population for later use
            ...
            db.query(sqlLanguages, [languagesList], (err, languageResults) => {
                ...
                res.render("language", { languages: languagesData });
            });
        });
    });
};
